...
if [[ -z "$DATA_DIR" ]]; then
    ...
    check_gold_table_duckdb() {
        ...
        if $DUCKDB_AVAILABLE; then
            ...
            local row_count
            row_count=$(duckdb -c "INSTALL delta; LOAD delta; SELECT COUNT(*) FROM delta_scan('${table_path}');" 2>/dev/null | grep -oE '[0-9]+' | tail -1 || echo "ERROR")
    ...
    if [[ "$STAGE" -eq 3 ]]; then
        ...
        for table in "${STREAM_TABLES[@]}"; do
            ...
            if [[ "$PARQUET_COUNT" -gt 0 ]]; then
                if $DUCKDB_AVAILABLE; then
                    ROW_COUNT=$(duckdb -c "INSTALL delta; LOAD delta; SELECT COUNT(*) FROM delta_scan('${TABLE_PATH}');" 2>/dev/null | grep -oE '[0-9]+' | tail -1 || echo "0")
